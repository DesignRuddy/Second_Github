...
var replyService = (function() {	
	...
	// getList() 함수는 param이라는 객체를 통해서 필요한 파라미터를 전달받아서
	// JSON 목록을 호출합니다. JSON 형태가 필요하므로 URL 호출 시 확장자를 '.json'으로 요구합니다.
	...
	function update(reply, callback, error) {
			console.log("RNO 댓글번호 =" + reply.rno);
   ...
   // 그리고, get.jsp에서는 댓글의 번호만을 전달 처리 합니다.
	...
	function displayTime(timeValue) {
		...
		// getMonth() is zero-based 이기 때문에 +1을 처리해줌.
		var mm= dateObj.getMonth() + 1;
		var dd= dateObj.getDate();
		
		// 24시간이 지나지 않은
		if(gap < (1000 * 60 * 60 * 12)){ // 앞서 gap 변수값이 24시간[(ms*m*24)hh = 1000*60*60*24] 보다 작다면(지나지 않았다면)
			return [yy,'/',mm,'/',dd,' ',
					(hh> 9 ? '' : '0') + hh, ":",
					(mi> 9 ? '' : '0') + mi, ':',
					(ss> 9 ? '' : '0') + ss ].join('');
		// 24시간이 지난댓글(날짜 표시 예시 = 2022/03/22)
		} else{
			return [ yy, '/',
					(mm> 9 ? '' : '0') + mm, '/',
					(dd> 9 ? '' : '0') + dd ].join('');
